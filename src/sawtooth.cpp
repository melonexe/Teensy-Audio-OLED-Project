 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp0 = std::max<float>(1.1920929e-07f, std::fabs(fRec2[0]));
			float fTemp1 = fRec0[1] + fConst3 * fTemp0;
			...
			output0[i0] = FAUSTFLOAT(fRec3[0] * (2.0f * fRec1 + -1.0f));
			fRec2[1] = fRec2[0];
			fRec0[1] = fRec0[0];
			fRec3[1] = fRec3[0];
		}
	...
	#define FAUST_FILE_NAME "sawtooth.dsp"
	...
	FAUST_ADDNUMENTRY("freq", fEntry0, 4.4e+02f, 2e+01f, 2e+04f, 0.01f);
	FAUST_ADDNUMENTRY("gain", fEntry1, 1.0f, 0.0f, 1.0f, 0.01f);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, freq, "freq", fEntry0, 4.4e+02f, 2e+01f, 2e+04f, 0.01f) \
		p(NUMENTRY, gain, "gain", fEntry1, 1.0f, 0.0f, 1.0f, 0.01f) \
...
sawtooth::sawtooth() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void sawtooth::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }
